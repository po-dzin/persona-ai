...
const LEGAL_DOCS: Record<LegalDocId, { title: string; updatedAt: string; sections: Array<{ heading?: string; paragraphs: string[] }> }> = {
  privacy: {
    ...
    updatedAt: "5 апреля 2026",
    sections: [
      {
        heading: "1. Какие данные мы собираем",
        paragraphs: [
          "Persona обрабатывает данные Telegram-профиля (id, username, имя, аватар при наличии), технические события использования, а также фото и текстовые запросы, которые вы отправляете для генерации.",
          "Мы используем эти данные только для работы функций приложения: создание генераций, показ истории, начисление/списание монет, поддержка и защита от злоупотреблений.",
        ],
      },
      {
        heading: "2. Сроки хранения",
        paragraphs: [
          "Срок хранения исходных фото: до 48 часов. Срок хранения результатов генерации: до 30 дней.",
          "Метаданные заказов, транзакций и платежей могут храниться дольше в объеме, необходимом для учета, безопасности, антифрода и выполнения правовых обязанностей.",
        ],
      },
      {
        heading: "3. Передача и защита данных",
        paragraphs: [
          "Для оказания сервиса данные могут обрабатываться доверенными подрядчиками: AI-провайдерами, инфраструктурой хранения и платежными провайдерами.",
          "Мы применяем разумные технические и организационные меры защиты, но ни один способ передачи или хранения данных не может гарантировать абсолютную безопасность.",
        ],
      },
      {
        heading: "4. Ваши права",
        paragraphs: [
          "Вы можете обратиться в поддержку для уточнения, исправления или удаления данных в пределах, допускаемых законодательством и техническими ограничениями сервиса.",
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        paragraphs: [
          "Используя Persona (Telegram Mini App и web-версию), вы подтверждаете согласие с настоящими условиями.",
          "Вы обязуетесь использовать сервис только законно и в личных или коммерчески допустимых целях, не нарушающих права третьих лиц.",
        ],
      },
      {
        heading: "2. Правила контента",
        paragraphs: [
          "Вы подтверждаете, что имеете необходимые права и согласия на все загружаемые изображения.",
          "Запрещены: NSFW-контент, чужие фото без согласия, попытки имитации личности, использование материалов с нарушением авторских и иных прав.",
        ],
      },
      {
        heading: "3. Генерации и доступность",
        paragraphs: [
          "Стоимость генерации определяется выбранной AI-моделью и отображается в интерфейсе до запуска.",
          "Сервис предоставляется по модели «как есть». Мы можем изменять функциональность, лимиты и тарифы для развития и безопасности продукта.",
        ],
      },
      {
        heading: "4. Ограничение и прекращение доступа",
        paragraphs: [
          "Мы можем ограничить доступ к сервису полностью или частично при нарушении условий, риске злоупотребления или угрозах безопасности.",
        ],
      },
    ],
  },
  payments: {
    ...
    sections: [
      {
        heading: "1. Способы оплаты",
        paragraphs: [
          "Основной способ оплаты в приложении — Telegram Stars. В web-сценариях может использоваться дополнительный платежный провайдер.",
          "Платеж обрабатывается внешним провайдером; мы не храним полные данные банковских карт пользователей.",
        ],
      },
      {
        heading: "2. Монеты и списания",
        paragraphs: [
          "После успешной оплаты монеты зачисляются на баланс пользователя и используются для генераций в соответствии с текущими тарифами.",
          "Списание монет происходит при запуске генерации. Стоимость зависит от выбранной AI-модели.",
        ],
      },
      {
        heading: "3. Возвраты и компенсации",
        paragraphs: [
          "При технической ошибке генерации компенсация (refund монет) выполняется автоматически или через поддержку после проверки инцидента.",
          "Если запрос отклонен за нарушение контент-политики, возврат обычно не выполняется, кроме отдельных случаев по решению поддержки.",
        ],
      },
      {
        heading: "4. Споры и поддержка",
        paragraphs: [
          "По вопросам списаний, начислений и возвратов обращайтесь через раздел «Поддержка» в профиле.",
          "Сроки и условия возврата могут зависеть от правил платежного провайдера и применимого законодательства.",
        ],
      },
    ],
  },
  disclaimer: {
    ...
    sections: [
      {
        heading: "1. Результаты генерации",
        paragraphs: [
          "Генеративный результат является вероятностным и может отличаться от ожиданий, превью и реальных характеристик изображения.",
          "Мы не гарантируем художественную, коммерческую или иную пригодность каждого сгенерированного результата для конкретной цели.",
        ],
      },
      {
        heading: "2. Ограничение ответственности сервиса",
        paragraphs: [
          "Persona не несет ответственности за косвенные убытки, упущенную выгоду, репутационные потери или иные последствия использования либо невозможности использования сервиса.",
          "Мы не отвечаем за сбои, вызванные сторонними платформами, провайдерами, сетью, устройством пользователя или ограничениями Telegram/Web среды.",
        ],
      },
      {
        heading: "3. Ответственность пользователя",
        paragraphs: [
          "Пользователь несет ответственность за законность загружаемого контента, получение согласий и соблюдение прав третьих лиц.",
          "Перед публикацией, коммерческим использованием или передачей результатов третьим лицам пользователь обязан самостоятельно проверить правовые риски.",
        ],
      },
    ],
  },
...
export function App() {
  ...
  const transitionToScreen = useCallback(
    (screen: BaseScreen, options?: { closeTransientLayers?: boolean; immediate?: boolean }) => {
      ...
      if (options?.immediate || screenHandoffDelayMs <= 0) {
    ...
    <main className="app-shell" onClickCapture={handleUiTapHaptic}>
      ...
      <TabBar
        ...
        onChange={(screen) => {
          ...
          transitionToScreen(screen, { immediate: true });
        }}
