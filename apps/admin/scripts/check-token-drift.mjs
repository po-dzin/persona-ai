...
if (writeMode) {
  ...
  console.log(`Admin token-drift baseline updated: ${path.relative(process.cwd(), baselinePath)}`);
  process.exit(0);
}
...
if (violations.length > 0) {
  console.error("Admin token drift detected. Replace literals/hardcoded values with tokens or refresh baseline intentionally.");
...
console.log("Admin token drift check passed.");
